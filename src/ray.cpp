...
void Pixel::remap() {
	...
	x = (2 * ((x + 0.5) / WIDTH) - 1) * ANGLE * ASPECT_RATIO;
	y = (1 - 2.0 * ((y + 0.5) / HEIGHT)) * ANGLE;
...
vec3 Pixel::map() {
    ...
    float alpha = tan(FOVX/2.0) * ( (x - half_width) / half_width);
    float beta = tan(FOVY/2.0) * ( (half_height - y) / half_height);
